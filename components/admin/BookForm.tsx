...
export default function BookForm({ book, action }: BookFormProps) {
  const [insightsUa, setInsightsUa] = useState<string[]>(
    book?.key_insights_ua?.length ? book.key_insights_ua : ['', '', '']
  )
  const [practicalUa, setPracticalUa] = useState<string[]>(
    book?.practical_ua?.length ? book.practical_ua : ['', '', '']
  )
  const [reflectionUa, setReflectionUa] = useState<string[]>(
    book?.reflection_ua?.length ? book.reflection_ua : ['', '', '']
  )
  const [quotesUa, setQuotesUa] = useState<string[]>(
    book?.quotes_ua?.length ? book.quotes_ua : ['', '']
  )
  ...
  return (
    <form ref={formRef} className="space-y-6" onSubmit={(e) => e.preventDefault()}>
      ...
      <div className={sectionCls}>
        ...
        <div>
          ...
          <input name="title_ua" type="text" required defaultValue={book?.title_ua}
            className={inputCls} placeholder="Назва книги"
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-400">
            URL: /books/<span className="text-gray-600">{slugValue || '...'}</span>
          </p>
        </div>
      </div>
      ...
      <div className={sectionCls}>
        ...
        <div>
          <label className={labelCls}>Самарі *</label>
          <RichTextEditor
            value={summaryUa}
        ...
        <div className="space-y-3">
          {quotesUa.map((val, idx) => (
            <div key={idx} className="flex gap-2 items-start">
              ...
              />
              <button type="button"
                onClick={() => setQuotesUa(quotesUa.filter((_, i) => i !== idx))}
      ...
      <div className={sectionCls}>
        ...
        <div className="space-y-3">
          {insightsUa.map((val, idx) => (
            <div key={idx} className="flex gap-2 items-start">
              ...
              <button type="button"
                onClick={() => setInsightsUa(insightsUa.filter((_, i) => i !== idx))}
                className="text-gray-300 hover:text-red-400 transition-colors text-xl leading-none px-1 shrink-0 mt-2">
                ×
              </button>
            </div>
        ...
        <button type="button"
          onClick={() => setInsightsUa([...insightsUa, ''])}
          className="mt-3 text-sm text-[#2D5016] hover:underline flex items-center gap-1">
      ...
      <div className={sectionCls}>
        ...
        <div className="space-y-3">
          {practicalUa.map((val, idx) => (
            <div key={idx} className="flex gap-2 items-start">
              ...
              <button type="button"
                onClick={() => setPracticalUa(practicalUa.filter((_, i) => i !== idx))}
                className="text-gray-300 hover:text-red-400 transition-colors text-xl leading-none px-1 shrink-0 mt-2">
                ×
              </button>
            </div>
        ...
        <button type="button"
          onClick={() => setPracticalUa([...practicalUa, ''])}
          className="mt-3 text-sm text-[#2D5016] hover:underline flex items-center gap-1">
      ...
      <div className={sectionCls}>
        ...
        <div className="space-y-3">
          {reflectionUa.map((val, idx) => (
            <div key={idx} className="flex gap-2 items-start">
              ...
              />
              <button type="button"
                onClick={() => setReflectionUa(reflectionUa.filter((_, i) => i !== idx))}
                className="text-gray-300 hover:text-red-400 transition-colors text-xl leading-none px-1 shrink-0 mt-2">
                ×
              </button>
            </div>
        ...
        <button type="button"
          onClick={() => setReflectionUa([...reflectionUa, ''])}
          className="mt-3 text-sm text-[#2D5016] hover:underline flex items-center gap-1">
