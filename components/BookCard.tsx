...
export default function BookCard({ book, locale }: BookCardProps) {
  const tr = t[locale]
  ...
  return (
    <Link href={`/${locale}/books/${book.slug}`} className="group block">
        ...
        <div className="relative aspect-[2/3] bg-gray-100 overflow-hidden">
          ...
          <div className="absolute top-2 right-2 z-20">
            <div className="bg-white/90 backdrop-blur-sm rounded-full p-1.5 shadow-sm">
              <FavoriteButton book={book} locale={locale} />
            </div>
          </div>
          ...
          {mounted && isRead(book.id) && (
            <div className="absolute bottom-2 left-2 z-20 flex items-center gap-1 bg-[#2D5016] text-white text-xs px-2 py-1 rounded-full">
              ...
              <span>{locale === 'uk' ? 'Прочитано' : 'Read'}</span>
            </div>
          )}
        </div>
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-400">
              {book.read_time_min} {tr.minutesShort}
            </span>
          </div>
